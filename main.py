    ...
    try:
        ...
        if AshbyTalent.HOMEOSTAT in config["talents"]:
            try:
                ...
                if not valid: 
                    result["homeostat"] = {"status": "blocked", "reason": err}
                else:
                    res = system_state.apply_input(
                        event.get("severity", "medium"), 
                        event.get("type", "bug"), 
                        time.time(), 
                        event.get("trust_score", 1.0)
                    )
        ...
        if AshbyTalent.COUNTERFACTUAL in config["talents"] and "counterfactual" in event:
            try:
                ...
                if scenario not in DEMO_SCENARIOS: 
                    ...
                    result["counterfactual"] = engine.run(
                        cf_data.get("observed", {}), 
                        cf_data.get("intervention", {}), 
                        cf_data.get("target")
                    )
            except Exception as e: 
